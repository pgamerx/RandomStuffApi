...
dotenv.config();
const SfwWaifu = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const AuthKey = req.query.Authorization;
    if (!AuthKey)
        return res
            .status(403)
    ...
    const response = yield fetch(`https://waifu.pics/api/sfw/${type}`, {
        method: "GET",
    });
    const json = yield response.json();
    return res.json(json);
});
const NsfwWaifu = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const private_key = process.env.PRIVATE_KEY;
    const RapidApi = req.get("x-RapidApi-private") || req.get("RapidApi-private");
    if (RapidApi !== private_key)
        return res.status(400).send("You are only allowed to make requests through RapidApi, contact for more support.");
    ...
    if (!AuthKey)
        return res
            .status(403)
            .send("Authorization key is not present, contact for more support.");
    if (!(yield (0, plan_1.isPremium)(AuthKey)))
        return res
            .status(403)
            .send("Your key is invalid, contact for more support.");
    const type = req.query.type ? req.query.type : "waifu";
    const response = yield fetch(`https://waifu.pics/api/nsfw/${type}`, {
        method: "GET",
    });
    const json = yield response.json();
    return res.json(json);
});
exports.default = { NsfwWaifu, SfwWaifu };
