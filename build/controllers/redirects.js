"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const RapidApiRedirect = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    res.redirect('https://rapidapi.com/pgamerxdev/api/random-stuff-api/');
});
const DocsRedirect = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    res.redirect('https://api-docs.pgamerx.com');
});
const MainSiteRedirect = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    res.redirect("https://api-info.pgamerx.com");
});
const Deprecated = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    res.json(["Hey there, we have deprecated our old version and now we can only be operated through RapidApi. Kindly headover to https://api.pgamerx.com/new for more information"]);
});
const RegisterSiteRedirect = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    res.redirect("https://api-info.pgamerx.com/manage-key");
});
exports.default = {
    RapidApiRedirect,
    DocsRedirect,
    MainSiteRedirect,
    Deprecated,
    RegisterSiteRedirect
};
